...
async function connectToWhatsApp() {
    ...
    console.log(`[FLEKO-X BOT] Bot is starting...`);
    console.log(`[FLEKO-X BOT] Using Baileys v${version.join('.')}${isLatest ? ' (latest)' : ''}`);
    ...
    const sock = makeWASocket({
        ...
        auth: state,
        browser: ["FLEKO-X BOT", "Chrome", "1.0.0"]
    });
            ...
            for (const c of call) {
                if (c.status === 'offer') {
                    ...
                    if (config.ANTICALLMSG) {
                        ...
                        await sock.sendMessage(c.from, { text: "*FLEKO-X BOT: Calls are not allowed. Your call has been rejected.*" });
                    }
                }
            }
        }
    ...
    sock.ev.on('connection.update', (update) => {
        ...
        if (qr) {
            console.log('[FLEKO-X BOT] Scan the QR code below to connect:');
        }
        if (connection === 'close') {
            let reason = new Boom(lastDisconnect?.error)?.output.statusCode;
            if (reason === DisconnectReason.loggedOut) {
                console.log(`[FLEKO-X BOT] Device Logged Out, Please Scan Again and Run.`);
                process.exit();
            } else {
                connectToWhatsApp();
            }
        } else if (connection === 'open') {
            console.log('[FLEKO-X BOT] Connected Successfully!');
            const msg = `*FLEKO-X BOT Connected!*\n\n*Prefix:* ${config.PREFIX}\n*Mode:* ${config.MODE}\n*Owner:* ${config.OWNERNAME}`;
            ...
            if (isCmd) {
                ...
                switch (command) {
                    ...
                    case 'menu':
                        ...
                        break;
                    case 'repo':
                        await sock.sendMessage(from, { text: `*FLEKO-X BOT REPOSITORY* 📂\n\nClick here to view the source code:\n${config.REPOURL}` }, { quoted: m });
                        break;
                    case 'pair':
                        await sock.sendMessage(from, { text: `*FLEKO-X BOT PAIRING* 🔗\n\n*Note:* Please wait while the pairing link loads. It is designed to be long-lasting and stable.\n\nClick here to get your session code:\n${config.PAIRINGURL}` }, { quoted: m });
                        break;
                    case 'alive':
                        await sock.sendMessage(from, { text: `*FLEKO-X BOT is Alive!* 🚀\n\n*Owner:* ${config.OWNERNAME}\n*Bot Name:* ${config.BOTNAME}` }, { quoted: m });
                        break;
                    default:
                        break;
                }
